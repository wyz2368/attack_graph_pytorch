...
def run(
    ...
    # Train new best-response to opponent's new policy.
    opponent_mixture = np.array([1.0])

    logger.info("Training attacker's best-response to the defender's new policy.")
    att_best_response_dir = settings.get_attacker_best_response_dir()
    att_best_response_name = f"att_br_epoch1.pkl"
    _ = training.train(
        game, 1, opponent_mixture, 1, writer, osp.join(att_best_response_dir, att_best_response_name))
    logger.info("Done training the attacker's new best-response.")

    logger.info("Training defender's best-response to the attacker's new policy.")
    def_best_response_dir = settings.get_defender_best_response_dir()
    def_best_response_name = f"def_br_epoch1.pkl"
    _ = training.train(
        game, 0, opponent_mixture, 1, writer, osp.join(def_best_response_dir, def_best_response_name))
    ...
    for epoch_i in range(2, n_epochs+1):  # We do 1-indexing to be compliant with double-oracle code.
        ...
        #  - First we will create a new best-response to the equilibrium opponent
        #    by mixing the Q values according to the opponent's mixture.
        #  - Then we will *learn* new best-response policies to the opponents new
        #    strategies (the ones just constructed via mixing).
        nash_eq = game.nasheq[epoch_i - 1]

        # Create a new policy that is a best-response to the equilibrium opponent. To do this
        # we take best-responses to each of the opponent's pure-strategy and mix them according
        # to the opponent's equililbrium mixture.
        att_new_policy = QMixture(mixture=nash_eq[0], q_funcs=_load_all_best_responders(epoch_i, 1))
        def_new_policy = QMixture(mixture=nash_eq[1], q_funcs=_load_all_best_responders(epoch_i, 0))

        if perform_policy_distillation and epoch_i > 2:
            # Perform policy distillation, once we have replay buffers.
            att_new_policy = policy_distillation(
                teacher=att_new_policy,
                student_ctor=gin.REQUIRED,
                env=game.env,
                epoch=epoch_i,
                training_attacker=1)
            def_new_policy = policy_distillation(
                teacher=def_new_policy,
                student_ctor=gin.REQUIRED,
                env=game.env,
                epoch=epoch_i,
                training_attacker=0)
        ...
        att_policy_name = f"att_str_epoch{epoch_i}.pkl"
        torch.save(att_new_policy, osp.join(settings.get_attacker_strategy_dir(), att_policy_name))
        game.add_att_str(att_policy_name)

        def_policy_name = f"def_str_epoch{epoch_i}.pkl"
        torch.save(def_new_policy, osp.join(settings.get_defender_strategy_dir(), def_policy_name))
        game.add_def_str(def_policy_name)

        # Train new best-response to opponent's new policy.
        opponent_mixture = np.zeros_like(game.def_str, dtype=np.float32)
        opponent_mixture[-1] = 1.0

        logger.info("Training attacker's best-response to the defender's new policy.")
        att_best_response_dir = settings.get_attacker_best_response_dir()
        att_best_response_name = f"att_br_epoch{epoch_i}.pkl"
        _ = training.train(
            game, 1, opponent_mixture, epoch_i, writer, osp.join(att_best_response_dir, att_best_response_name))
        logger.info("Done training the attacker's new best-response.")

        logger.info("Training defender's best-response to the attacker's new policy.")
        def_best_response_dir = settings.get_defender_best_response_dir()
        def_best_response_name = f"def_br_epoch{epoch_i}.pkl"
        _ = training.train(
            game, 0, opponent_mixture, epoch_i, writer, osp.join(def_best_response_dir, def_best_response_name))
        logger.info("Done training the defender's new best-response.")
...
def _load_all_best_responders(epoch: int, is_attacker: bool) -> typing.List:
    ...
    :rtype: List
    """
    name = "att_br_epoch{}.pkl" if is_attacker else "def_br_epoch{}.pkl"
    save_dir = settings.get_attacker_best_response_dir() if is_attacker else settings.get_defender_best_response_dir()
