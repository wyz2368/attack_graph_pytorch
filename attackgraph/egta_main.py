...
from attackgraph import double_oracle, mixed_opponent, mixed_oracle
...
def main(argv):
    ...
    # Run EGTA.
    if FLAGS.algorithm == "double-oracle":
        double_oracle.run(load_env=FLAGS.env, env_name=None, n_processes=gin.REQUIRED)

    elif FLAGS.algorithm == "mixed-oracle":
        mixed_oracle.run(env_name=FLAGS.env, n_processes=gin.REQUIRED)

    elif FLAGS.algorithm == "mixed-opponent":
        mixed_opponent.run(env_name=FLAGS.env, n_processes=gin.REQUIRED)

    else:
        raise ValueError(f"Unknown algorithm: {FLAGS.algorithm}.")
